...
# Run schema.sql on first start (idempotent — IF NOT EXISTS / ON CONFLICT)
if [ -n "$LOCAL_DB_URL" ]; then
  echo "[engine] waiting for database..."
  ...
  if [ "$APPLIED" != "yes" ]; then
    echo "[engine] applying schema.sql..."
    node -e "
      const fs = require('fs');
      const {Client} = require('pg');
      ...
      c.connect()
        .then(() => c.query(sql))
        .then(() => c.end())
        .then(() => { console.log('[engine] schema ready'); process.exit(0); })
        .catch((e) => { console.error('[engine] schema error:', e.message); process.exit(0); });
    " || true
  fi
fi
